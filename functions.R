...
create_multileveldata_D1<- function(cluster_num = 300, # number of clusters
                                    ...
                                    tau_var = 0.2, # variance of the cluster level tau
                                    clustereffect=TRUE){
  ...
  while (balanced == FALSE) {
    ...
    if (clustereffect == FALSE) {
      ...
      pop$Y0  <- 4 + 0.4*(pop$X11  + pop$X12 + pop$X13 + pop$S1 + pop$X21 -
                            pop$X22 + pop$X23 - pop$S2 + pop$X14) + E + 1*pop$U_j
...
create_multileveldata_D2 <- function(cluster_num = 150, # number of clusters
                                     cluster_size_min = 20,
                                     cluster_size_max = 50,
                                     R_var = 0.6653, # variance of residual
                                     V_var = 0.8,  # variance of cluster effect in selection
                                     U_var = 0.0776, # variance of cluster effect in outcome
                                     tau_var = 0.2, # variance of tau
                                     clustereffect=TRUE) {
  
  # This D2 DGP is based on the D1 v7 version.
  ...
  while (balanced == FALSE){
    ...
    pop <- data.frame(id, X11, X12, X13, S1, X14, 
                      X21=X21[id], X22=X22[id], X23=X23[id], S2=S2, S2_2 = S2_2[id],
    ...
    if (clustereffect == FALSE) {
      ...
      pop$lps <- -0.1 + 0.2*(pop$X11 + pop$X12 + pop$X13 + pop$S_is_1 +pop$S_is_2+ pop$X14) +
        -0.2*(pop$X21 + pop$X22  + pop$X23 - pop$S_is_3 + pop$S2_2)  + pop$R_j
      ...
      pop$Y1 <- pop$Y0 + 0.8 +
        ...
        0.4 * pop$X22 +
        0.2 * pop$X23 +
        0.1 * pop$X14 - 0.2 * pop$S2_2 + pop$tau_var_j
    }
