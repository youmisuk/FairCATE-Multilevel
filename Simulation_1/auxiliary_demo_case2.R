...
run_iteration_2 <- function(iter){
  ...
  df <- create_multileveldata_D1(cluster_num = 300, # number of clusters
                                 ...
                                 U_var = 1.2, # 0.3 variance of cluster effect in outcome
                                 tau_var = 0.2, # 0.2 variance of cluster effect in treatment
                                 U_var_coef = 1,
                                 clustereffect=TRUE)
  ...
  # get the relative utility(RU) for true policy
  ...
  beta_hat_setting1 <- ml_fr_out_setting1$beta.hat[-length(ml_fr_out_setting1$beta.hat)]
  ...
  beta_hat_setting2 <- ml_fr_out_setting2$beta.hat[-length(ml_fr_out_setting2$beta.hat)]
...
results <- future_lapply(1:500, run_iteration_2)


# Convert list of lists into a data frame
results_df2 <- do.call(rbind, lapply(results, function(res) {
  data.frame(
...
save(results_df2, file = "results/results_df_case_2.rdata")

# get the averaged results
output_mean <- colMeans(results_df2, na.rm = TRUE)
