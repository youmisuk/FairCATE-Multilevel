...
run_iteration_3 <- function(iter){
  ...
  df <- create_multileveldata_D1(cluster_num = 300, # number of clusters
                                 cluster_size_min = 10,
                                 cluster_size_max = 50,
                                 R_var = 1, #0.6653 variance of residual
                                 V_var = 1,  # 0.8 variance of cluster effect in selection
                                 U_var = 1, # 0.3 variance of cluster effect in outcome
                                 U_V_same = TRUE,
...
results <- future_lapply(1:500, run_iteration_3)


# Convert list of lists into a data frame
results_df3 <- do.call(rbind, lapply(results, function(res) {
  data.frame(
...
save(results_df3, file = "results/results_df_case_3.rdata")

# get the averaged results
output_mean <- colMeans(results_df3, na.rm = TRUE)
